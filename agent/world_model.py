...
class WorldModel(nn.Module):
    def __init__(self,
        ...
        # Determine feature dimension for the prediction heads.
        ...
        # Instantiate dynamics (RSSM). Note: RSSM expects time-first inputs.
        self.dynamics = RSSM(
        ...
        # Create optimizer for the world model.
        ...
        # Create a slow target network for the critic if enabled.
        ...
        for key, value in observations.items():
            if key == "action":
                if hasattr(self.configuration, "action_space_type") and self.configuration.action_space_type == "continuous":
                    ...
                    action_tensor = torch.tensor(value, device=self.configuration.computation_device, dtype=torch.long).clone().detach()
                    if getattr(self.configuration, "debug", False):
                ...
                processed[key] = torch.tensor(value, device=self.configuration.computation_device, dtype=torch.float32).clone().detach()
    ...
    def train_step(self, data: Dict[str, Any]) -> Tuple[Dict[str, torch.Tensor], Dict[str, Any], Dict[str, float]]:
        ...
        with torch.amp.autocast("cuda", enabled=self.use_amp):
            # Encoder outputs batch-first [B, T, embed_dim]; transpose to time-first for RSSM.
            ...
            # Get features; expect batch-first [B, T, f].
        ...
        context = {
            "embedding": embeddings,  # Still time-first.
            "features": self.dynamics.get_features(posterior),
            "kl": kl_value
        }
        if getattr(self.configuration, "debug", False):
    ...
    def generate_video(self, data: Dict[str, Any]) -> torch.Tensor:
        data = self.preprocess(data)
        embeddings = self.encoder(data).transpose(0, 1)  # [T, B, embed_dim]
